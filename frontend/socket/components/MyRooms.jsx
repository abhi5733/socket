  ...
  const[room,setRoom] = useState("") // for maintaining the room name
   ...
   return <>  {
        info.rooms.length>0?<Box> {info.rooms.map((el)=>{
          ...
          <Button ml={2} size={"xs"} onClick={()=>{setGroup(el.roomName),joinRoom(el.roomName)}}>Join</Button>
          </Flex>
      </Box> 
        }) }
       ...
       {newRoom &&  <Box mt={5}>
        ...
        <Button size={["sm","xs","xs","xs"]} onClick={()=>{joinRoom(room);setRoom("")}} >Join</Button>
        <Button size={["sm","xs","xs","xs"]} ml={[2,0,0,0]} onClick={()=>setNewRoom(false)} >Back</Button>
        </Flex>
      </Box>  }
         { !newRoom && <Button  size={"xs"} mt={[2,5,5,5]}  onClick={()=> setNewRoom(true)} >Join New Room</Button> }
